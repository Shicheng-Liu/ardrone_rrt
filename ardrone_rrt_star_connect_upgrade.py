...
pygame.display.set_caption('ardrone_rrt_star_connect_upgrade')
white = 255, 255, 255
black = 0, 0, 0
red = 255, 0, 0
green = 0, 255, 0
    ...
    if event.type == pygame.QUIT:
      ...
      if count== 1:
        ...
        for i in range(vertex_number):
          ...
          pygame.display.flip() 
          for x in tree:
              if distance([x.x,x.y],[new_vertex_1.x,new_vertex_1.y])<distance([new_vertex.x,new_vertex.y],[new_vertex_1.x,new_vertex_1.y]) and distance([x.x,x.y],[new_vertex_1.x,new_vertex_1.y])<radius:
                new_vertex=x                  
